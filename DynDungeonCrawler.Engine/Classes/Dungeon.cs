...
namespace DynDungeonCrawler.Classes
{
    ...
    public class Dungeon
    {
        ...
        private async Task LoadEnemyNamesAsync()
        {
            try
            {
                string userPrompt = $"Generate a simple JSON list (no explanations, no other text) of 10 fantasy-themed enemy names appropriate for the following dungeon theme: \"{theme}\".";
                string response = await _llmClient.GetResponseAsync(userPrompt, "You are an enemy name generator. You only respond with raw JSON list of names. Return only plain text, don't use markdown.");
